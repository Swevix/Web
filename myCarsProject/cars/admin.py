...
class CarAdmin(admin.ModelAdmin):
    ...
    fields = ['title', 'slug', 'description', 'price', 'manufacturer', 'tags']
    # Автоматическая генерация поля slug на основе поля title.
    prepopulated_fields = {"slug": ("title",)}
    # Поля, которые доступны только для чтения (например, автоматически генерируемые значения).
    readonly_fields = ['time_create', 'time_update']

    # Настройка списка записей (ранее выполненная):
    list_display = (
        'id',
        'title',
        'manufacturer',
    ...
    # Пользовательское действие для установки статуса "Опубликовано" (п.8)
    ...
    # Пользовательское действие для установки статуса "Черновик" (п.8)
